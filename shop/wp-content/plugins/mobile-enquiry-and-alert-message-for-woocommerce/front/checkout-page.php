<?php 
/**
 * Defines if checkout page
 */
function mmwea_is_checkout_page() {
...
/**
 * Shortcode for checkout page button
 */
add_shortcode('mmwea_checkout_wh_btn','mmwea_checkout_shortcode',10);
function mmwea_checkout_shortcode() {
    if(mmwea_is_checkout_page()){
        $general_settings_options   = get_option('mmwea_general_settings_options');
        $checkout_page_options        = get_option('mmwea_product_checkout_page_options');
...
function mmwea_checkout_page_front_setting() {
    if( mmwea_is_checkout_page() && is_checkout() ) {
        $btn_position_hook      = "";
        $checkout_page_options  = get_option('mmwea_product_checkout_page_options');
        ...
        add_action($btn_position_hook, function() {
            _e(do_shortcode('[mmwea_checkout_wh_btn]'));
        });
    }
}