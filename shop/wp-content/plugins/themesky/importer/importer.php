...
if( !class_exists('TS_Importer') ){
	class TS_Importer{
		...
		function import_page_setup( $default_settings ){
			...
			$default_settings['page_title']  = esc_html__( 'Yobazar - Import Demo Content' , 'themesky' );
			$default_settings['menu_title']  = esc_html__( 'Yobazar Importer' , 'themesky' );
			$default_settings['capability']  = 'import';
			$default_settings['menu_slug']   = 'yobazar-importer';
		...
		function change_url(){
			...
			$import_url = 'https://demo.theme-sky.com/yobazar-import';
		...
		function set_elementor_site_settings(){
			...
			if( $id ){ /* Fixed width, space, ... if query does not return the imported post */
				...
				$page_settings['space_between_widgets']['size'] = 16;
		...
		function update_product_category_id_in_homepage_content(){
			...
			$pages = array(
					'Home'	=> array(
							'ids' => array(
								'17,31,34,35,36,37'
								, array( 'Shoes', 'Glasses', 'Shirts', 'Trousers', 'Swimwear', 'Hats' )
							)
					)
					,'Home 2'	=> array(
							'ids' => array(
		...
		function update_mega_menu_content(){
			...
			$mega_menus = array(
				'Shop'		=> array(29 => 'Men', 15 => 'Women')
			);
			
			foreach( $mega_menus as $title => $product_cats ){
				$mega_menu_post = get_page_by_title( $title, OBJECT, 'ts_mega_menu' );
				if( is_object( $mega_menu_post ) ){
					foreach( $product_cats as $old_id => $cat_name ){
						$cat = get_term_by( 'name', $cat_name, 'product_cat' );
						if( isset($cat->term_id) ){
							$old_string = '"parent":"' . $old_id . '"';
							$new_string = '"parent":"' . $cat->term_id . '"';
			...
			/* Update Nav Menu */
			$mega_menus = array(
				'Pages'		=> array(83 => 'Pages - Homes', 81 => 'Pages - Shop', 82 => 'Pages - Other')
			);
		...
		function update_footer_content(){
			...
			$footers = array(
				'Footer 5'		=> array(29 => 'Men', 15 => 'Women')
				,'Footer 4'		=> array(29 => 'Men', 15 => 'Women')
				,'Footer 3'		=> array(29 => 'Men', 15 => 'Women')
				,'Footer 2'		=> array(29 => 'Men', 15 => 'Women')
				,'Footer 1'		=> array(29 => 'Men')
			);
			
			foreach( $footers as $title => $product_cats ){
