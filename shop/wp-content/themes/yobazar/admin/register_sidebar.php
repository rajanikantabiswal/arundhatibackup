<?php 
function yobazar_get_list_sidebars(){
	$default_sidebars = array(
						array(
							'name' => esc_html__( 'Home Sidebar', 'yobazar' ),
							'id' => 'home-sidebar',
						...
						,array(
							'name' => esc_html__( 'Blog Sidebar', 'yobazar' ),
							'id' => 'blog-sidebar',
							'before_widget' => '<section id="%1$s" class="widget-container %2$s">',
							'after_widget' => '</section>',
							'before_title' => '<div class="widget-title-wrapper"><a class="block-control" href="javascript:void(0)"></a><h3 class="widget-title heading-title">',
						...
						,array(
							'name' => esc_html__( 'Blog Detail Sidebar', 'yobazar' ),
						...
						,array(
							'name' => esc_html__( 'Product Category Sidebar', 'yobazar' ),
						...
						,array(
							'name' => esc_html__( 'Filter Widget Area', 'yobazar' ),
						...
						,array(
							'name' => esc_html__( 'Product Detail Sidebar', 'yobazar' ),
	...
	$custom_sidebars = yobazar_get_custom_sidebars();
...
function yobazar_register_widget_area(){
	$default_sidebars = yobazar_get_list_sidebars();
	foreach( $default_sidebars as $sidebar ){
		register_sidebar($sidebar);
	}
}
add_action( 'widgets_init', 'yobazar_register_widget_area' );

/* Custom Sidebar */
add_action( 'sidebar_admin_page', 'yobazar_custom_sidebar_form' );
function yobazar_custom_sidebar_form(){
?>
	<form action="<?php echo admin_url( 'widgets.php' ); ?>" method="post" id="ts-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_attr_e('Custom Sidebar Name', 'yobazar'); ?>" />
		<input type="hidden" id="ts_custom_sidebar_nonce" value="<?php echo wp_create_nonce('ts-custom-sidebar'); ?>" />
        <button class="button-primary" id="ts-add-sidebar"><?php esc_html_e('Add Sidebar', 'yobazar'); ?></button>
    </form>
<?php
}

function yobazar_get_custom_sidebars(){
...
add_action('wp_ajax_yobazar_add_custom_sidebar', 'yobazar_add_custom_sidebar');
function yobazar_add_custom_sidebar(){
	if( isset($_POST['sidebar_name']) ){
		...
		if( $result ){
			die( esc_html__('Successfully added the sidebar', 'yobazar') );
		}
		else{
			die( esc_html__('Error! It seems that the sidebar exists. Please try again!', 'yobazar') );
		}
	}
	die('');
}

add_action('wp_ajax_yobazar_delete_custom_sidebar', 'yobazar_delete_custom_sidebar');
function yobazar_delete_custom_sidebar(){
	if( isset($_POST['sidebar_name']) ){
		check_ajax_referer('ts-custom-sidebar', 'sidebar_nonce');
		...
		die( esc_html__('Successfully deleted the sidebar', 'yobazar') );
	}
	die('');
}
?>